...
-- 커서(CURSOR) : SELECT문 또는 DML과 같은 SQL구문 실행했을 때
--				  해당 SQL을 처리하는 정보를 저장한 메모리 공간

-- SELECT INTO : 조회결과가 하나일 때 사용
...
--	명시적 커서 선언
	CURSOR c1 IS 
		...
		FROM DEPT;
BEGIN
	-- CURSOR 열기, FETCH, CLOSE는 자동으로 실행
	FOR c1_row IN c1
	LOOP
...
-- 트리거 : 데이터베이스 안의 특정 상황이나 동작,
--			이벤트가 발생하는 경우 자동으로 실행되는 기능 정의
...
CREATE TABLE EMP_TRG_LOG(
	...
	USER_NAME VARCHAR2(30),	-- DML 수행한 USER 이름
