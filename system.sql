...
-- 권한 권리
-- 1) 시스템 권한
--		사용자 생성, 수정, 삭제, 데이터베이스 접근, 오라클 데이터베이스 객체 생성 관리
--		ex) 데이터베이스 접속 권한(GRANT CREATE SESSION TO ....)
-- 2) 객체 권한
--		특정 사용자가 생성한 테이블/인덱스/뷰/시퀀스 등과 관련된 권한
--		ex) 데이터베이스 접속 권한(GRANT SELECT, INSERT, DELETE ON BOARD TO ....)

-- 권한 취소
-- REVOKE CREATE TABLE FROM C##TEST1;
-- REVOCE SELECT, INSERT, DELETE ON BOARD FROM C##TEST1;

-- C##TEST1 : 접속, 테이블 권한 부여
GRANT CREATE SESSION, CREATE TABLE TO C##TEST1;

-- 롤 : 여러 권한들의 묶음 정의
-- CREATE TABLE, CREATE VIEW, CREATE SESSION... => 원래 CONNECT 롤에 들어있었음
--	1) CONNECT 롤 : CREATE SESSION만 가지고 있음
--	2) RESOURCE 롤 : CREATE TABLE, CREATE SEQUENCE, CREATE TRIGGER, CREATE PROCEDURE 권한
-- 사용자 생성 후 CONNECT, RESOURCE 롤 두개 부여
